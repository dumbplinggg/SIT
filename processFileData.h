...
std::vector<float> generate_test_data_2d_grid(
    size_t width, size_t height,
    float noise_level, float outlier,
    size_t step_x, size_t step_y)
{
    ...
    for (size_t y = 0; y < height; ++y) {
        for (size_t x = 0; x < width; ++x) {
            ...
            // Выбросы на регулярной сетке
            if (step_x > 0 && step_y > 0 &&
                x % step_x == 0 && y % step_y == 0 &&
                !(x == 0 && y == 0))   // пропускаем левый верхний угол
            {
                float direction = (signal >= 0) ? 1.0f : -1.0f;
                noise += outlier * direction;
            }
